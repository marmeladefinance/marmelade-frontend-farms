...
const ActionPanel: React.FunctionComponent<ActionPanelProps> = ({ details, apr, multiplier, liquidity }) => {
  ...
  let getLink = ''
  if (farm.isTokenOnly) {
    getLink = `https://exchange.marmelade.finance/#/swap?outputCurrency=${tokenAddresses[process.env.REACT_APP_CHAIN_ID]}`
    ...
    getLink = `https://exchange.marmelade.finance/#/add/${getLiquidityUrlPathParts({ quoteTokenAdresses, quoteTokenSymbol, tokenAddresses })}`
  }
