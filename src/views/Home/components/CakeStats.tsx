...
const CakeStats = () => {
  ...
  let marmelPerBlock = 0;
  if(farms && farms[0] && farms[0].marmelPerBlock){
    marmelPerBlock = new BigNumber(farms[0].marmelPerBlock).div(new BigNumber(10).pow(18)).toNumber();
  }
    ...
    <StyledCakeStats>
      <CardBody>
        <Heading size="xl" mb="24px">
          {TranslateString(534, 'Marmel Stats')}
        </Heading>
        <Row>
        ...
        <Row>
          <Text fontSize="14px">{TranslateString(540, 'New MARMEL/block')}</Text>
          <Text bold fontSize="14px" color="textSubtle">{marmelPerBlock}</Text>
        </Row>
      </CardBody>
    </StyledCakeStats>
  )
